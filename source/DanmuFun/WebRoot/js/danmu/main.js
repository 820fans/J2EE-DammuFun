...
(function ($) {
	...
	var DanmuPlayer = function (element, options) {
		...
		this.$element.append('<div id="danmu-text-wrapper" class="danmu-div" id="' + $(element).attr("id") + '-danmu-div" ></div>');
		...
		this.$element.append('<video id="danmu-video-entity" class="danmu-video" src="' + options.src + '" width="' + options.width + '" height="' + options.height + '"></video>');
		...
		this.$ctrlMain.append('<div class="volume ctrl-btn-right"><span class="glyphicon glyphicon-volume-up"></span></div>');
		...
		this.getDanmu = function () {
			$.post(that.options.urlToGetDanmu, {}, function(data) {

				data=JSON.parse(data);

				for(i=0;i<data.length;i++)
					$("#danmup .danmu-div").danmu("addDanmu",[
				{ "text": data[i].text ,color:data[i].color,size:data[i].size,position:data[i].position,time:data[i].time}
					                                          ]);

				//alert(document.getElementById("danmu-video-entity").currentTime)
...
//				}
		...
		this.playPause = function (e) {
			if (e.data.video.paused) {
				...
				if(!showed){
					...
					$.post("viewVideo.html", {videoId:videoId}, function(data) {
		...
		//调整透明度事件--->改为调整音量
		$(this.id + " .danmu-op").on('mousemove', {that: that}, function (e) {
			document.getElementById("danmu-video-entity").volume=(e.target.value / 100);
...
//			$(e.data.that.id + " .danmaku").css("opacity", (e.target.value / 100));
		...
		$(this.id + " .full-screen").on("click", {video: this.video, that: that}, function (e) {
			if (!e.data.that.danmuPlayerFullScreen) {
				...
				//弹幕层全屏
				var elem = document.getElementById('danmup');
